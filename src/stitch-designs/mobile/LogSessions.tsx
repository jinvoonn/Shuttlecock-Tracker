...
export default function MobileLogSessions({ tubes, players }: MobileLogSessionsProps) {
  ...
  return (
    <div className="bg-[#f6f8f7] dark:bg-[#0a130e] font-['Lexend',_sans-serif] text-slate-900 dark:text-slate-100 min-h-screen flex flex-col antialiased pb-48">
      <div className="relative flex min-h-screen w-full flex-col max-w-[480px] mx-auto border-x border-slate-200 dark:border-slate-800 shadow-2xl bg-white dark:bg-[#0a130e]">
        {/* Header */}
        <header className="sticky top-0 z-30 bg-white/80 dark:bg-[#0a130e]/80 backdrop-blur-md border-b border-[#13ec80]/20 px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => router.back()} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors">
              <ArrowLeft className="text-[#13ec80] size-6" />
            </button>
            <h1 className="text-2xl font-black italic tracking-tighter uppercase">Log Session</h1>
          </div>
          <div className="bg-[#13ec80]/10 text-[#13ec80] border border-[#13ec80]/20 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest leading-none">Live</div>
        </header>
        ...
        <main className="p-4 space-y-8 flex-1 text-left">
          ...
          {/* Shuttle Counter */}
          <section className="bg-white dark:bg-[#0f172a] border-2 border-slate-100 dark:border-slate-800 rounded-3xl p-8 shadow-xl relative overflow-hidden">
            <div className="absolute top-0 right-0 size-32 bg-[#13ec80]/5 dark:bg-[#13ec80]/10 rounded-full -translate-y-16 translate-x-16 blur-3xl"></div>
            <h2 className="text-[10px] font-black text-[#13ec80] uppercase tracking-[0.2em] mb-6 text-center relative z-10">Shuttle Counter</h2>
            <div className="flex items-center justify-between gap-4 relative z-10">
              <button 
                onClick={() => setShuttlesUsed(Math.max(0, shuttlesUsed - 1))}
                className="size-16 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 border-2 border-slate-200 dark:border-slate-700 flex items-center justify-center rounded-2xl active:scale-90 transition-all shadow-sm"
              >
                <Minus className="size-8 font-black text-slate-600 dark:text-slate-400" />
              </button>
              <div className="flex flex-col items-center">
                <span className="text-8xl font-black text-slate-900 dark:text-white font-mono tracking-tighter tabular-nums">{shuttlesUsed.toString().padStart(2, '0')}</span>
                <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] mt-2">Shuttles Used</span>
              </div>
              <button 
                onClick={() => setShuttlesUsed(shuttlesUsed + 1)}
                className="size-16 bg-[#13ec80] flex items-center justify-center rounded-2xl active:scale-90 transition-all shadow-lg shadow-[#13ec80]/30 group"
              >
                <Plus className="size-8 font-black text-[#0a130e] group-active:scale-110" />
              </button>
            </div>
          </section>
          ...
          <section className="flex flex-col gap-4">
            <div className="flex items-center justify-between px-1">
              <h2 className="text-[10px] font-black text-[#13ec80] uppercase tracking-[0.2em]">Attendees ({attendeeIds.size}/{players.length})</h2>
              <button className="text-[10px] font-black text-[#13ec80] border-2 border-[#13ec80] px-3 py-1 rounded-lg uppercase tracking-[0.15em] active:scale-95 transition-transform" onClick={() => router.push('/view/record-transaction-stitch')}>ADD PAYMENT</button>
            </div>
            
            {/* Search Input */}
            <div className="relative group">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 size-5 transition-colors group-focus-within:text-[#13ec80]" />
              <input 
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                className="w-full bg-white dark:bg-[#0f172a] border-2 border-slate-100 dark:border-slate-800 rounded-2xl py-4 pl-12 pr-4 text-sm font-bold placeholder:text-slate-500 text-slate-900 dark:text-slate-100 focus:border-[#13ec80] focus:ring-4 focus:ring-[#13ec80]/5 transition-all outline-none shadow-sm" 
              ...
              {filteredPlayers.map((player) => (
                ...
                >
                  ...
                  <label className="relative inline-flex items-center cursor-pointer pointer-events-none">
                    ...
                    <div className="w-12 h-7 bg-slate-200 dark:bg-slate-700 rounded-full peer peer-checked:bg-[#13ec80] after:content-[''] after:absolute after:top-[4px] after:left-[4px] after:bg-white after:rounded-full after:h-[19px] after:w-[19px] after:transition-all peer-checked:after:translate-x-full"></div>
                  </label>
                </div>
              ))}
            </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 z-50">
          {/* Summary Bar */}
          <div className="bg-[#13ec80] px-6 py-5 flex items-center justify-between text-[#0a130e] shadow-[0_-12px_40px_rgba(0,0,0,0.3)]">
            <div className="flex gap-10">
              <div className="flex flex-col text-left">
                <span className="text-[10px] font-black uppercase tracking-[0.2em] opacity-60 leading-none">Total</span>
                <span className="text-2xl font-black font-mono tracking-tighter tabular-nums mt-1">${totalCost.toFixed(2)}</span>
              </div>
              <div className="flex flex-col text-left">
                <span className="text-[10px] font-black uppercase tracking-[0.2em] opacity-60 leading-none">Per Player</span>
                <span className="text-2xl font-black font-mono tracking-tighter tabular-nums mt-1">${perPlayerCost.toFixed(2)}</span>
              </div>
            </div>
            <button 
              disabled={isSubmitting}
              onClick={handleSubmit}
              className={`px-8 py-4 rounded-xl font-black text-xs uppercase tracking-[0.2em] active:scale-95 transition-all shadow-lg hover:brightness-110 ${isSubmitting ? 'bg-slate-800 text-slate-500' : 'bg-[#0a130e] text-[#13ec80]'}`}
            >
          ...
          {/* Bottom Nav */}
          <nav className="flex bg-white dark:bg-[#0a130e] border-t border-slate-100 dark:border-slate-800 px-2 pb-8 pt-4 shadow-xl">
            <button onClick={() => router.push('/view/dashboard-stitch')} className="flex flex-1 flex-col items-center gap-1.5 text-slate-400 dark:text-slate-500 hover:text-[#13ec80] transition-colors group">
            ...
            <button onClick={() => router.push('/view/sessions-stitch')} className="flex flex-1 flex-col items-center gap-1.5 text-[#13ec80] transition-colors group">
            ...
            <button onClick={() => router.push('/view/stock-inventory-stitch')} className="flex flex-1 flex-col items-center gap-1.5 text-slate-400 dark:text-slate-500 hover:text-[#13ec80] transition-colors group">
            ...
            <button onClick={() => router.push('/view/payment-ledger-stitch')} className="flex flex-1 flex-col items-center gap-1.5 text-slate-400 dark:text-slate-500 hover:text-[#13ec80] transition-colors group">
