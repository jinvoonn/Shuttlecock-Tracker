...
export default function EloRatingCard({ playerElo, placementMatchesPlayed = 5, eloLabel, eloColor, eloBg }: EloRatingCardProps) {
    ...
    return (
        ...
        >
            <div className="absolute top-4 right-1 opacity-10 blur-[0.5px] transform group-hover:scale-110 transition-transform pointer-events-none">
                <RankBadgeIcon rank={rank.name} size="large" className="scale-[2.5]" />
            </div>
            ...
            <div className="flex flex-col gap-1 w-full mb-4">
                <div className="flex items-center justify-between relative z-10">
                    <p className="text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 leading-none shadow-sm" style={{ color: rank.color }}>
                        <RankBadgeIcon rank={rank.name} size="small" className="scale-75" /> {rank.name}
                    </p>
